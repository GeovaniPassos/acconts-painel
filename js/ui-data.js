...
export function renderExpensesItem(expense) {
    ...
    //TODO: Ajustar o botão de pagamento, na lista é span, no forma é button, ajustar a função
    const statusClass = isPaid ? "btn-pendent" : "btn-paid";
    const statusText = isPaid ? "Pago" : "Pendente";

    let button = statusClass;
    console.log(button.dataset.pago);

    toggleStatusVisual(button, isPaid);
...
export function toggleStatusVisual(button, isPaid) {
    ...
    // Atualiza as cores
    if (isPaid) {
        button.classList.add("btn-paid");
        button.classList.remove("btn-pendent");
    } else {
        button.classList.add("btn-pendent");
        button.classList.remove("btn-paid");
    }
}
